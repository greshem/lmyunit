//12:11 2010-8-9 
//#2010_08_11_10:24:32 add by qzj
...
{
        fprintf (stderr, ("Usage: %s [-bfhixV] [-ob superblock] "
                 "[-oB blocksize] device\n"), program_name);
        exit (1);
}
...
static void list_desc (ext2_filsys fs)
{
        ...
        //char * tmp = (char *)(fs->block_map->bitmap);
        //char * block_bitmap = (char *)(fs->block_map->bitmap);
        //char * inode_bitmap = (char *)(fs->inode_map->bitmap);

        char * block_bitmap =NULL;// fs->block_map->bitmap;
        char * inode_bitmap =NULL;// fs->inode_map->bitmap;
        ...
        for (i = 0; i < fs->group_desc_count; i++) {
                ...
                printf (("Group %lu: (Blocks "), i);
                printf(range_format, group_blk, next_blk - 1);
                fputs(")\n", stdout);
                if (ext2fs_bg_has_super (fs, i)) {
                        printf (("  %s Superblock at "),
                                i == 0 ? ("Primary") : ("Backup"));
...
static void list_bad_blocks(ext2_filsys fs, int dump)
{
        ...
        if (retval) 
		{
                ...
                printf("ext2fs_read_bb_inode error %d", retval);
                return;
        }
        retval = ext2fs_badblocks_list_iterate_begin(bb_list, &bb_iter);
        if (retval) {
                printf("ext2fs_badblocks_list_iterate_begin %d %s ", retval,
            ...
            (jsb->s_header.h_blocktype !=
             (unsigned) ntohl(JFS_SUPERBLOCK_V2))) {
                com_err(program_name, 0,
                         ("Couldn't find journal superblock magic numbers"));
                exit(1);
        }
...
int main (int argc, char ** argv)
{
        ...
        if (retval) {
               // printf (program_name, retval,  ("while trying to open %s"),
                //         device_name);
        ...
        if (print_badblocks) {
                ...
                if (retval) {
                        //com_err (program_name, retval,
                          printf       ("while trying to read the bitmaps");
                         //        device_name);
                        ext2fs_close (fs);
                        exit (1);
                }
